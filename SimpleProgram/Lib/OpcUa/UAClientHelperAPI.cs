...
//            try
//            {
                if (_mApplicationConfig == null)
                {
                    _mApplicationConfig = await CreateClientConfiguration();
                }

                //Secify application configuration
                var applicationConfig = _mApplicationConfig;
                ...
                var endpointDescription = CreateEndpointDescription(url, secPolicy, msgSecMode);
...
//                _mSession.KeepAlive += Notification_KeepAlive;                
//            }
//            catch (Exception e)
//            {
//                //handle Exception here
...
//            }
        ...
        public string GetNamespaceUri(uint index)
        {
            ...
            if (_mSession.NamespaceUris.Count > index)
            {   //Get the uri for the namespace index
                return _mSession.NamespaceUris.GetString(index);
            }
            else
            {
                var e = new Exception("Index is out of range");
                throw e;
            }
        }
        ...
        public uint GetNamespaceIndex(string uri)
        {
            ...
            var namespaceIndex = _mSession.NamespaceUris.GetIndex(uri);
            ...
            {
                var e = new Exception("Namespace doesn't exist");
                throw e;
            }
        }
        ...
        {
            var namespaceArray = new List<string>();

            //Read all namespace uris and add the uri to the list
            for (uint i = 0; i < _mSession.NamespaceUris.Count; i++)
            {
                namespaceArray.Add(_mSession.NamespaceUris.GetString(i));
            }
        ...
        public ReferenceDescriptionCollection BrowseNode(ReferenceDescription refDesc)
        {
            ...
            try
            {
                ...
                {
                    _mSession.BrowseNext(null, false, continuationPoint, out revisedContinuationPoint, out nextreferenceDescriptionCollection);                    
        ...
        public ReferenceDescriptionCollection BrowseNodeByReferenceType(ReferenceDescription refDesc, NodeId refTypeId)
        {
            ...
            {
                //Browse from starting point for all object types
                _mSession.Browse(null, null, nodeId, 0u, BrowseDirection.Forward, refTypeId, true, 0, out continuationPoint, out referenceDescriptionCollection);
        ...
        public List<string[]> ReadStructUdt(String nodeIdString)
        {
            ...
            String parseString;
            String xmlString = GetTypeDictionary(nodeIdString, _mSession, out parseString);
        ...
        public void WriteStructUdt(String nodeIdString, List<string[]> dataToWrite)
        {
            ...
            try
            {
                _mSession.Write(null, valuesToWrite, out results, out diag);
            }
            catch (Exception e)
            {
                //Handle Exception here
        ...
        public List<string> GetMethodArguments(String nodeIdString)
        {
            ...
            try
            {
                ...
                if (methodNode.NodeClass == NodeClass.Method)
                {
                    ...
                    _mSession.Browse(null, null, nodeId, 0u, BrowseDirection.Forward, ReferenceTypeIds.HasProperty, true, 0, out continuationPoint, out referenceDescriptionCollection);
        ...
        public IList<object> CallMethod(String methodIdString, String objectIdString, List<string[]> inputData)
        {
            ...
            //Call the method
            outputArguments = _mSession.Call(objectNodeId, methodNodeId, inputArguments);
        ...
        private static async Task<ApplicationConfiguration> CreateClientConfiguration()
        {
            ...
            {
                ApplicationName = "UA Client 1500.1",
            ...
            configuration.SecurityConfiguration = new SecurityConfiguration
            {
                ApplicationCertificate =
                    ...
                    {
                        StorePath = "CurrentUser\\My", SubjectName = configuration.ApplicationName
                    },
                TrustedIssuerCertificates = {StorePath = "CurrentUser\\Root"},
                TrustedPeerCertificates = {StorePath = "CurrentUser\\Root"}
            };
            ...
            var clientCertificate = await configuration.SecurityConfiguration.ApplicationCertificate.Find(true);

            // create a new self signed certificate if not found.
            if (clientCertificate == null)
            {
                // Get local interface ip addresses and DNS name
                var localIps = GetLocalIpAddressAndDns();

                const ushort keySize = 2048; //must be multiples of 1024
                const ushort lifeTime = 24; //in month
                const ushort algorithm = 1; //0 = SHA1; 1 = SHA256
            ...
            {
                OperationTimeout = 360000, MaxStringLength = 67108864, MaxByteStringLength = 16777216
            };
        ...
        private static List<string[]> ParseDataToTagsFromDictionary(List<Object> varList, byte[] byteResult)
        {
            ...
            foreach (object val in varList)
            {
                ...
                {
                    var tempArray = new long[arraylength];
                ...
                {
                    var tempArray = new int[arraylength];
                    for (var i = 0; i < arraylength; i++)
                    {
                ...
                {
                    var e = new Exception("Data type is too complex to be parsed." + System.Environment.NewLine + "Please avoid array of UDT/Struct inside UDT/Struct");
                    throw e;
                }
        ...
        /// <exception cref="Exception">Throws and forwards any exception with short error description.</exception>
        private static long GetLengthOfDataToWrite(List<string[]> dataToWrite)
        {            
            Int64 length = 0;
            Int32 arraySize = 0;
            foreach (string[] val in dataToWrite)
            {
                ...
                else if (val[2] == "Int32" && val[0].Contains("_Size"))
                {
                    ...
                    {
                        var tempStringArr = ((string[])dataToWrite[dataToWrite.IndexOf(val) + 1])[1].Split(';');
                ...
                {
                    var e = new Exception("Unknow data type. Can't determine length of data");
                    throw e;
                }
            }
            return length;
        ...
        private static byte[] ParseDataToByteArray(List<string[]> dataToWrite, Int64 dataLength)
        {
            ...
            foreach (string[] val in dataToWrite)
            {
                ...
                {
                    var e = new Exception("Can't covert " + val[0] + ".");
                    throw e;
                }
            }
            return bytesToWrite;
